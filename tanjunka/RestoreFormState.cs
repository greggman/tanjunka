...
namespace Tanjunka
{
    public class PersistWindowState : System.ComponentModel.Component
    {
        ...
        public Form Parent
        {
            set
            {
                ...
                this.m_regPath = "Software\\" + this.m_parent.CompanyName + "\\" + System.Reflection.Assembly.GetEntryAssembly().GetName().Name;
        ...
        private void OnClosing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            string KeyPrefix = this.m_parent.GetType().Namespace+"."+this.m_parent.GetType().Name;
            key.SetValue(KeyPrefix+"Left", m_normalLeft);
            key.SetValue(KeyPrefix+"Top", m_normalTop);
            key.SetValue(KeyPrefix+"Width", m_normalWidth);
            key.SetValue(KeyPrefix+"Height", m_normalHeight);
            ...
            key.SetValue(KeyPrefix+"WindowState", (int)m_windowState);
        ...
        private void OnLoad(object sender, System.EventArgs e)
        {
            ...
            if(key != null)
            {
                int left = (int)key.GetValue(KeyPrefix+"Left", m_parent.Left);
                int top = (int)key.GetValue(KeyPrefix+"Top", m_parent.Top);
                int width = (int)key.GetValue(KeyPrefix+"Width", m_parent.Width);
                int height = (int)key.GetValue(KeyPrefix+"Height", m_parent.Height);
                FormWindowState windowState = (FormWindowState)key.GetValue(KeyPrefix+"WindowState", (int)m_parent.WindowState);
    ...
    public class AppForm : System.Windows.Forms.Form
    {
        ...
        public AppForm()
        {
            ...
            m_windowState.RegistryPath = @"Software\YourCompany\YourApp"; // set registry path in HKEY_CURRENT_USER
