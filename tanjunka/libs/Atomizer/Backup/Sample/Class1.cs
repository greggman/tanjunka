...
namespace AtomizerSample
{
	...
	class Class1
	{
		...
		static void Main(string[] args)
		{
			...
			if (args.Length < 2)
			{
				Console.WriteLine("Usage: app.exe <username> <password> [typepad | blogger] [image filename]");
				return;
			}
			...
			if (args.Length < 3)
			{
				Console.WriteLine("Defaulting to blogger.com");
				Console.WriteLine("");
			}
			else
			{
				if (args[2].Equals("typepad"))
				{
			...
			foreach (service service in services)
			{
				...
				if (service.srvType.Equals(serviceType.post))
				{
					...
					foreach (linkType link in entry.links)
					{
						...
						{
							atom.DeleteBlogEntry(link.href);
							Console.WriteLine("Your entry \"" + entry.title + "\" was deleted from " + service.name + "!");
							Console.WriteLine("");
							break; // no need to go further, we've done it.
						}
					}
				}
